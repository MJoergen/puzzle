...
/**************************************************************************
 **************************************************************************/
void CSolver::CreateBitMaps(vector<CBitMap>& bitmaps, const CBlock& block, const CSquare& offset)
{
...
void CSolver::BuildBitMaps(int in_rows, int in_cols)
{
    ...
    for (unsigned int block_no=0; block_no<m_orientations.NumBlocks(); block_no++)
    {
        ...
        cout << "Considering block " << block_no << endl;
        TRACE( "Considering block " << block_no << endl);
        for (int row=0; row<m_rows; row++)
        {
            for (int col=0; col<m_cols; col++)
            {
        ...
        cout << "Total of " << m_bitmaps[block_no].size() << " bitmaps." << endl;
        TRACE( "Total of " << m_bitmaps[block_no].size() << " bitmaps." << endl);
    }
} /* end of BuildBitMaps */
...
void CSolver::PlaceBitMaps(CBitMap bitmap, unsigned int first_block, unsigned int num_blocks)
{
    ...
    TRACE( "bitmap = " << bitmap << ", first_block=" << first_block << endl);

    m_stats.nodes++;
    if (m_stats.nodes == NODES_PER_DOT)
    {
        m_stats.nodes = 0;
        m_stats.dots++;
        cerr << ".";
...
#ifdef STATISTICS
    m_stats.examine_tests[num_blocks]++;
#endif
    ...
    for (block_temp = first_block; block_temp < m_bitmapIndex.size(); block_temp++)
    {
        ...
        const vector<CBitMap>& bitmapsForBlock = m_bitmaps[block_temp];
        ...
        TRACE( "Block " << block_temp << " has " << found << " legal moves." << endl);

        // If a block has no legal places, then abort immediately.
        if (!found)
        {
#ifdef STATISTICS
            m_stats.examine_nolegal[num_blocks]++;
#endif
            TRACE( "Discarded." << endl);
            return;
        }
    ...
    {
#ifdef STATISTICS
        m_stats.examine_cutoffs[num_blocks]++;
#endif
        TRACE( "Discarded because board could not be filled completely." << endl);
        return;
    } /* end of if */
    ...
    {
#ifdef STATISTICS
        m_stats.examine_oneblock[num_blocks]++;
#endif
        TRACE( "bitmapTemp2=" << bitmapTemp2 << endl);
        TRACE( "Some squares can only be covered by a single block." << endl);
        ...
        for (block_temp = first_block; block_temp < m_bitmapIndex.size(); block_temp++)
        {
            ...
            for (unsigned int i=0; i<bitmapsForBlock.size(); i++)
            {
                ...
                {
                    TRACE( "Found block " << block_temp << ", bitmap " << i << endl);
                    if (first_block == block_temp)
                        first_block++;
    ...
    {
        cerr << "Success! count=" << m_stats.solutions << ", nodes=" << m_stats.nodes << 
            ", dots=" << m_stats.dots <<
            ", completed " << (m_bitmapIndex[0]*100)/m_bitmaps[0].size() << "%" << endl;
        GenerateBoard();
        cout << m_board;
...
#ifdef STATISTICS
    m_stats.examine_recurse[num_blocks]++;
#endif
    ...
    TRACE( "Searching next level with block " << block_temp << endl);
    const vector<CBitMap>& bitmapsForBlock = m_bitmaps[block_temp];
    for (unsigned int i=0; i<bitmapsForBlock.size(); i++)
    {
        if (bitmap.AreBitsDistinct(bitmapsForBlock[i]))
        {
            TRACE( "Using bitmap " << i << " " << bitmapsForBlock[i] << endl);
            m_bitmapIndex[block_temp] = i;
